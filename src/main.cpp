...
#if defined(NDEBUG)
# error "EPRCOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x594af034a55a5c4a253a665250ce6323bc0f70d5e0ad4f794f808689f44c0848");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EPRCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EPRCOIN Signed Message:\n";
...
{
    // EPRCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 405000 blocks
    nSubsidy >>= (nHeight / 405000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60;
static const int64 nTargetSpacing = 12 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EPRCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EPRCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1505158213, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Create at 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e35f8137f779376e2385ad6526acfce6542df01323927e7acd9ed84161d8e44c5bcb238a8328bf7edb949d8c528249c1f1921b66bb2596651542c1756a65cc2c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1505158213;
        ...
        assert(block.hashMerkleRoot == uint256("0x0993af9c50cc37bf30c4dc3412b0e36d42f9ffd0e3cee15e85bc3d466558a20f"));
...
unsigned char pchMessageStart[4] = { 0x2f, 0x3d, 0xcf, 0xc4 }; // EPRCOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EPRCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EPRCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EPRCOINMiner(CWallet *pwallet)
{
    printf("EPRCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eprcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EPRCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EPRCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EPRCOINMiner, pwallet));
}
